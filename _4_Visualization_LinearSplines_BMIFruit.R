####################################################################################  
######################### VISUALIZATION WITH LINEAR SPLINE ######################### 
####################################################################################  
...
packages <- c("foreach","doParallel","boot","rmutil","mvtnorm","gam","sandwich","ggplot2", "SuperLearner","xgboost",
              ...
              "sl3", "tlverse", "tmle3", "beepr", "lspline")
...
pree_fruit <- readRDS(file="filepath_tmle_object")
dat <- read.csv(file="data", header=TRUE, sep=",")
...
#-------------------------------------------------------------------------------------------------------------------------------------
# ADDITIONAL STEP: LINEAR SPLINES 
#-------------------------------------------------------------------------------------------------------------------------------------
plot2 <- lm(rd_mean ~ lspline(bmi, c(22, 32, 45)), data=plot_dat)
summary(plot2)

# create data frame to hold predictions
dat2 <- data.frame(bmi = seq(from = min(plot_dat$bmi), to = max(plot_dat$bmi), by = 1))

dat2$pred <- predict(plot2, newdata = dat2)
...
plot <- ggplot(plot_dat) + 
  geom_smooth(aes(y = rd_mean, x = bmi),color="black", alpha = 1, size = 1.2, se=F) +
  geom_ribbon(aes(ymin = sd_minus, ymax =sd_plus, x = bmi), linetype = 2, size = 1.2, color="gray20", alpha = 0.1) +
        labs(x = "BMI (kg/m^2)",
       y = "Adjusted risk difference") +
  geom_hline(yintercept = 0,color="gray",size=2, linetype=5) +
  theme_bw()+
...
plot + layer(geom = "line", data = dat2, stat = "identity", 
             position = "identity", aes(x = bmi, y = pred),
             params = list(color="red", size=1, alpha = 0.6)) 
